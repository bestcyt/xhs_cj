...
<div class="layui-card">
    <div class="layui-card-body" style="padding: 15px;">
        <form class="layui-form" method="post" action="<?php echo U('setting/role/edit');?>" id="tenant_form">
            <input type="hidden" name="id" value="<?php echo $role_row['id'];?>">
            <div class="layui-form-item">
                <div class="layui-inline">
                    <label class="layui-form-label required">名称</label>
                    <div class="layui-input-inline">
                        <input type="text" name="name" lay-verify="required" autocomplete="off" placeholder="请输入名称" class="layui-input" maxlength="20" value="<?php echo $role_row['name'];?>" style="width: 500px;">
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <table class="layui-table api_access_table" lay-even lay-skin="line">
                    ...
                    <?php echo recycle_tree_list($value,$role_row["nodes"]);?>
                    </tbody>
                </table>
                <?php endforeach;?>
            </div>
...
<?php
//递归格式
function recycle_tree_list(array $data,&$nodes){
    $str="";
    foreach($data as $value){
        $buffer_left=($value['level']-1)*3+1;
        $checked = in_array($value["id"], $nodes) ? "checked" : "";
        if($value["children"]){
            ...
            $str.='<td class="tree_level_a"><input type="checkbox" name="node_ids[]" value="'.$value["id"].'" lay-skin="primary" class="tree_level_abc" '.$checked.'></td>';
            $str.='</tr>'.recycle_tree_list($value["children"],$nodes);
        }else{
